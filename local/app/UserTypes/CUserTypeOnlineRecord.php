...
class CUserTypeOnlineRecord
{
	 ...
	 */
	 public static function getUserTypeDescription()
	 {
		 return [
			...
			'GetPropertyFieldHtml' => [self::class, 'GetPropertyFieldHtml'],  //метод отображения свойств в Админке
			'GetPublicViewHTML' => [self::class, 'GetPublicViewHTML'], //метод отображения значений свойства в Публичной части
			'GetPublicEditHTML' => [self::class, 'GetPropertyFieldHtml'], //метод отображения значений в форме редактирования
		];
	...
	public static function GetPublicViewHTML($arProperty, $arValue, $strHTMLControlName)
	{
		...
		function LazyBanner(name, date, procedure, doctor){
			...
			BX.ajax({
				url: "/local/ajax/hendlers_userfields_online_records.php",  //файл обработчика на который идет запрос
				method: "POST",  // метод запроса GET/POST
				// параметры передаваемые запросом
				data: {
					NAME: name,
					TIME: date,
					PROC_ID: procedure,
		...
		BX.PopupWindowManager.create("bookingPopup_" + pr_id, pr_id, {
				...
				titleBar: "Запись на прием",
				closeByEsc: true, //закрывать при нажатии esc apeshellarg
